  ...
  return (
    <AccordionItem value={organization.id} className="border-none">
      <AccordionTrigger
        ...
        className={cn(
          "flex items-center gap-x-2 p-1.5 text-neutral-400 rounded-md hover:bg-neutral-500/10 transition text-start no-underline hover:no-underline",
          isExpanded && "bg-neutral-500/10"
        )}
      >
        <div className="flex items-center gap-x-2">
          ...
          <span className="font-medium text-sm text-neutral-100">{organization.name}</span>
        </div>
      </AccordionTrigger>
      
      <AccordionContent className="pt-1 text-neutral-400">
        {routes.map((route) => {
          // Logic: Boards must be exact match, others allow sub-paths
          const isRouteActive = route.label === "Boards" 
            ? pathname === route.href 
            ...
            <Button
              ...
              className={cn(
                ...
                isRouteActive 
                  ...
                  : "text-neutral-400 hover:text-neutral-100 hover:bg-neutral-500/10"
              )}
              variant="ghost"
            >
              {route.icon}
  ...
  return (
    <div className="flex items-center gap-x-2">
      <div className="w-10 h-10 relative shrink-0">
        <Skeleton className="h-full w-full absolute bg-neutral-800" />
      </div>
